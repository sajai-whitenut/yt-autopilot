...
{{
  "title": "catchy SEO title under 55 chars, create curiosity or shock",
  ...
  "thumbnail_bg_query": "cinematic dramatic background scene related to {NICHE}, no people, no text",
  "broll": ["specific visual term 1","specific visual term 2","specific visual term 3","specific visual term 4","specific visual term 5","specific visual term 6","specific visual term 7","specific visual term 8"]
}}"""
...
print(f"✅ voice.mp3 saved — duration: {audio_duration:.1f}s ({audio_duration/60:.1f} min)")
...
for term in data["broll"]:
    for page in [1, 2]:
        r = requests.get(
            ...
            headers=headers,
            params={"query": term, "per_page": 3, "page": page, "orientation": "landscape", "size": "medium"}
        )
        videos = r.json().get("videos", [])
        for video in videos:
            files = video["video_files"]
            # Prefer HD files for quality
            ...
            with open(clip_path, "wb") as f:
                f.write(requests.get(chosen["link"]).content)
            clips.append(clip_path)
            print(f"  ✅ clip_{len(clips)-1}: {term}")
            if len(clips) >= 20:
                break
        if len(clips) >= 20:
            break
...
print(f"✅ Downloaded {len(clips)} clips total")

# ── STEP 4: RENDER VIDEO WITH FFMPEG ────────────────
print("⚙️ Rendering video with FFmpeg...")

# Write concat list — repeat clips until we have enough duration
...
# ── STEP 5: GENERATE THUMBNAIL WITH FFMPEG ───────────
print("🖼️ Generating thumbnail...")

# Download background from Pexels photos
bg_r = requests.get(
    "https://api.pexels.com/v1/search",
    headers={"Authorization": PEXELS_KEY},
    params={"query": data["thumbnail_bg_query"], "per_page": 1, "orientation": "landscape"}
)
photos = bg_r.json().get("photos", [])
if photos:
    bg_url = photos[0]["src"]["large2x"]
    bg_data = requests.get(bg_url).content
    with open("bg.jpg", "wb") as f:
        f.write(bg_data)
    bg_file = "bg.jpg"
else:
    # Fallback: use first video frame
    subprocess.run(["ffmpeg", "-i", "broll.mp4", "-vframes", "1", "-q:v", "2", "bg.jpg", "-y"], check=True)
    bg_file = "bg.jpg"

# Build thumbnail: dark overlay + bold text using FFmpeg drawtext
thumb_text = data["thumbnail_text"].upper().replace("'", "").replace(":", "")
# Split into two lines if long
words = thumb_text.split()
mid = len(words) // 2
line1 = " ".join(words[:mid]) if mid > 0 else thumb_text
line2 = " ".join(words[mid:]) if mid > 0 else ""

drawtext = (
    f"drawtext=text='{line1}':fontsize=90:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-60"
    f":borderw=6:bordercolor=black@0.9:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,"
    f"drawtext=text='{line2}':fontsize=90:fontcolor=#FFD700:x=(w-text_w)/2:y=(h-text_h)/2+60"
    f":borderw=6:bordercolor=black@0.9:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
)
...
subprocess.run([
    ...
    "-vf", f"scale=1280:720:force_original_aspect_ratio=increase,crop=1280:720,"
           f"colorchannelmixer=rr=0.4:gg=0.4:bb=0.4,{drawtext}",
